...
from PyQt5.QtWidgets import (QApplication, QDialog, QHBoxLayout, QProgressBar,
...
class CustomMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.label.setText(_translate(
            'Dialog', 'PORT checking for signal ....'))
        ...
        self.btn_setDAC3.setText(_translate(
            'Dialog', 'Set Loops in 1-st motor'))
        self.btn_setDAC4.setText(_translate(
            'Dialog', 'Set Loops in 2-nd motor'))
        ...
        time.sleep(0.3)
        self.setDAC()
        self.myDataLoop = threading.Thread(
            name='myDataLoop', target=dataSendLoop, daemon=True, args=(self.addData_callbackFunc, self.p))
    ...
    def setDAC(self):
        sendData = b'\x03'
        heater = b'\x00\x01'
        sendData += self.spinBox.value().to_bytes(2, byteorder='big',
                                                  signed=True) + heater + b'\x00\x00'
    ...
    def stopMoving(self):
        sendData = b'\x04'
        sendData += (0).to_bytes(2, byteorder='big', signed=True) + (0).to_bytes(2,
                                                                                 byteorder='big', signed=True) + (0).to_bytes(2, byteorder='big', signed=True)
        calc_crc = crc16.crc(sendData)
        sendData = b'\x06\x85' + sendData + \
            calc_crc.to_bytes(2, byteorder='big', signed=False)
        self.p.write(sendData)
...
class CustomFigCanvas(FigureCanvas, TimedAnimation):
    ...
    def __init__(self):
        ...
        self.line1_head = Line2D([], [], color='blue',
                                 marker='o', markeredgecolor='blue')
        self.ax1.add_line(self.line1)
        self.ax1.add_line(self.line1_tail)
        self.ax1.add_line(self.line1_head)
        self.ax1.set_xlim(0, self.xlim - 1)
        self.ax1.set_ylim((SCALE_DOWN - 5000) *
                          SCALE_GRAPH_VAL, SCALE_UP * SCALE_GRAPH_VAL)
        ...
        if wait_bytes >= 7:
            ...
            if s_byte == '\x85':
                ...
                if calc_crc == rec_crc[0]:
                    ...
                    writeData.append(
                        (time.time() - start_time, -adc_val[0] * SCALE_GRAPH_VAL))
